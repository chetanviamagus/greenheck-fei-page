...
(function ($) {
  ...
  $(function () {
    var formContainers = $(
      '[data-sf-role="form-container"]:has([data-sf-role="ajax-submit-url"])'
    );
    formContainers.each(function (i, element) {
      var formContainer = $(element);
      var loadingImg = formContainer.find('[data-sf-role="loading-img"]');
      var fieldsContainer = formContainer.find(
        '[data-sf-role="fields-container"]'
      );
      var successMessage = formContainer.find(
        '[data-sf-role="success-message"]'
      );
      var errorMessage = formContainer.find('[data-sf-role="error-message"]');
      var generalErrorMessage = formContainer.find(
        '[data-sf-role="general-error-message"]'
      );
      var redirectUrl = formContainer
        .find('input[data-sf-role="redirect-url"]')
        .val();
      var submitUrl = formContainer
        .find('input[data-sf-role="ajax-submit-url"]')
        .val();

      // Create modal HTML
      var modalHtml = `
                <div id="modal-popup" class="ck modal" style="display:none;">
      ...
      var submitClickHandler = function () {
        ...
        if (parentForm.length > 0) parentFormChildren.unwrap();
        ...
        newForm.one("submit", function () {
          ...
          if (typeof MarketoSubmitScript !== "undefined") {
            MarketoSubmitScript._populateFormId(
              $(newForm).find('input[data-sf-role="form-id"]').val()
            );
            var newSubmitButtons = $(newForm).find(
              MarketoSubmitScript._settings.externalFormSubmitButtonsQuery
            );
            if (newSubmitButtons.length > 0)
              MarketoSubmitScript._formFields =
                MarketoSubmitScript._getExternalFormFields(newSubmitButtons[0]);
            if (
              MarketoSubmitScript._formFields &&
              MarketoSubmitScript._formFields.length === 0
            )
          ...
          if (wrapped) formContainer.unwrap();

          if (parentForm.length > 0) parentFormChildren.wrapAll(parentForm);
...
(function ($) {
  ...
  var CaptchaRestApi = function (rootUrl) {
    this.rootUrl =
      rootUrl && rootUrl[rootUrl.length - 1] !== "/" ? rootUrl + "/" : rootUrl;
  };
  ...
  CaptchaWidget.prototype = {
    ...
    captchaAudio: function () {
      return this.getOrInitializeProperty("_captchaAudio", "captcha-audio");
    },
    captchaAudioBtn: function () {
      return this.getOrInitializeProperty(
        "_captchaAudioBtn",
        "captcha-audio-btn"
      );
    },
    ...
    captchaDataInvalidAnswerMessage: function () {
      return this.getOrInitializeProperty(
        "_captchaDataInvalidAnswerMessage",
        "captcha-iam"
      );
    },
    ...
    captchaRefresh: function () {
      ...
      self.restApi.getCaptcha().then(function (data) {
        if (data) {
          ...
          self
            .captchaAudio()
            .attr("src", "data:audio/wav;base64," + data.Audio);
          self.captchaAudioBtn().click(function () {
            self.captchaAudio()[0].play();
          });
    ...
    showInvalidMessage: function () {
      ...
      if (this.errorMessage().length > 0) {
        ...
        this.getElementByDataSfRole("invalid-captcha-input").css(
          "visibility",
          "visible"
        );
      }
  ...
  function getValidationMessages(input) {
    var container = $(input).parents(
      '[data-sf-role="field-captcha-container"]'
    );
    var validationMessagesInput = $(container).find(
  ...
  function _getErrorMessageContainer(input) {
    ...
    if (container) {
      var errorMessagesContainer = container.querySelector(
  ...
  function changeOrInput(e) {
    if (typeof e.target.validity == "undefined") return;
...
(function ($) {
  var changeOrInput = function (e) {
    ...
    var hasChecked =
      $(container).find('input[data-sf-role="checkboxes-field-input"]:checked')
        .length > 0;

    if (
      hasChecked ||
      container.find('[data-sf-role="required-validator"]').val() === "False"
    ) {
  ...
  $(function () {
    ...
    var checkboxClickHandler = function (e) {
      ...
      var checkboxes = container.find(
        'input[data-sf-role="checkboxes-field-input"]'
      );
      var otherInput = $(
        container.find('[data-sf-checkboxes-role="other-choice-text"]').first()
      );
      var otherCheckbox = $(
        container
          .find('[data-sf-checkboxes-role="other-choice-checkbox"]')
          .first()
      );
      var otherCheckboxIndex = checkboxes.index(otherCheckbox);
      var currentIndex = checkboxes.index($(e.target));
      var isRequired =
        container.find('[data-sf-role="required-validator"]').val() === "True";

      if (otherCheckbox.is(":checked")) {
        otherInput.attr("type", "text");

        if (isRequired) otherInput.attr("required", "required");
        else otherInput.removeAttr("required");
      } else {
    ...
    for (var i = 0; i < containers.length; i++) {
      ...
      if (
        container.find('[data-sf-role="required-validator"]').val() ===
          "True" &&
        !inputs.is(":checked")
      )
        $(inputs[0]).attr("required", "required");
...
(function ($) {
  ...
  var checkRequired = function (container) {
    ...
    if (container.is(":hidden")) return true;
  ...
  var checkFileTypes = function (container, fileTypes) {
    ...
    var inputs = container.find('input[type="file"]');
    for (var i = 0; i < inputs.length; i++) {
      var violationMessage = $(inputs[i])
        .closest('[data-sf-role="single-file-input"]')
        .find('[data-sf-role="filetype-violation-message"]');
      if (inputs[i].value) {
  ...
  var checkFileSizes = function (container, min, max) {
    ...
    for (var i = 0; i < inputs.length; i++) {
      var violationMessage = $(inputs[i])
        .closest('[data-sf-role="single-file-input"]')
        .find('[data-sf-role="filesize-violation-message"]');
  ...
  var formValidation = function (ev) {
    var requiredValidationResult =
      !ev.data.config.IsRequired || checkRequired(ev.data.container);
    var fileTypesValidationResult =
      ev.data.config.AcceptedFileTypes.length === 0 ||
      checkFileTypes(ev.data.container, ev.data.config.AcceptedFileTypes);
    var fileSizesValidationResult =
      !(ev.data.config.MinFileSizeInMb || ev.data.config.MaxFileSizeInMb) ||
      checkFileSizes(
        ev.data.container,
        ev.data.config.MinFileSizeInMb * 1024 * 1024,
        ev.data.config.MaxFileSizeInMb * 1024 * 1024
      );

    return (
      requiredValidationResult &&
      fileTypesValidationResult &&
      fileSizesValidationResult
    );
  };
  ...
  var init = function (element) {
    ...
    var inputTemplate = jElement
      .find('[data-sf-role="file-input-template"]')
      .html();
    ...
    jElement.find('input[type="file"]').data("sfvalidator", function () {
      return formValidation({
        data: { config: config, container: inputContainer },
      });
    });
...
(function ($) {
  var changeOrInput = function (e) {
    var container = $(e.target).parents(
      '[data-sf-role="multiple-choice-field-container"]'
    );
  ...
  $(function () {
    ...
    var radioClickHandler = function (e) {
      ...
      var radios = container.find(
        'input[data-sf-role="multiple-choice-field-input"]'
      );
      var otherInput = $(
        container
          .find('[data-sf-multiple-choice-role="other-choice-text"]')
          .first()
      );
      var otherRadio = $(
        container
          .find('[data-sf-multiple-choice-role="other-choice-radio"]')
          .first()
      );
...
(function ($) {
  ...
  $(function () {
    ...
    var initializeFormContainer = function (element) {
      ...
      var navigationFieldContainers = formElement.find(
      ...
      var modifySrOnlyData = function (currentPage, totalPages, pageTitle) {
        ...
        var stepOfResource = $(element)
          .find('[data-sf-role="step-of-resources"]')
          .val();
        var valueText =
          stepOfResource
      ...
      var updateNavigationFields = function (navigationElements, index) {
        navigationElements.each(function (navIndex, navigationElement) {
          ...
          var progressPercent = formElement.find(
            '[data-sf-role="progress-percent"]'
          );
          ...
          if (pages && pages.length > 0) {
            pages.each(function (i, page) {
              ...
              var activeCssClass =
                $(navElement).attr("data-sf-active-css-class") || "active";
              var pastCssClass =
                $(navElement).attr("data-sf-past-css-class") || "past";
              var futureCssClass =
                $(navElement).attr("data-sf-future-css-class") || "future";

              var pastIndicatorPast = $(page).find(
                "[data-sf-progress-indicator='past']"
              );
              var pastIndicatorIncomplete = $(page).find(
                "[data-sf-progress-indicator='incomplete']"
              );
              ...
              $(page)
                ...
                .removeClass(futureCssClass)
                .removeClass(pastCssClass);
    ...
    var initializeFormContainer = function (element) {
      ...
      var getSkipToPageItem = function (pageIndex) {
        ...
        for (var i = 0; i < skipToPageCollection.length; i++) {
          if (skipToPageCollection[i].SkipFromPage === pageIndex)
            return skipToPageCollection[i];
        }
      ...
      var getSkipFromPageItem = function (pageIndex) {
        var item = null;
        for (var i = 0; i < skipToPageCollection.length; i++) {
          if (skipToPageCollection[i].SkipToPage === pageIndex)
            return skipToPageCollection[i];
        }
      ...
      separatorsNext.click(function (e) {
        ...
        tryGoToNextStep(currentStepContainer, function () {
          ...
          formElement.trigger("form-page-changed", [
            formStepIndex,
            previousIndex,
          ]);
          focusForm();
...
(function ($) {
  $(function () {
    ...
    function init() {
      ...
      for (var i = 0; i < containers.length; i++) {
        var textarea = $(containers[i]).find(
      ...
      $(this)
        ...
        .submit(function (ev) {
          if (!ev.isDefaultPrevented()) {
            ...
            var firstPageBreak = $(ev.target).find(
              '[data-sf-btn-role="next"]:visible'
            )[0];
...
(function ($) {
  $(function () {
    ...
    function invalid(e) {
      ...
      var isValidLength =
        e.target.value.length >= validationRestrictions.minLength;

      if (validationRestrictions.maxLength > 0)
        isValidLength &=
          e.target.value.length <= validationRestrictions.maxLength;
    ...
    function getValidationRestrictions(input) {
      var container = $(input).parents('[data-sf-role="text-field-container"]');
      var validationRestrictionsInput = $(container).find(
        '[data-sf-role="violation-restrictions"]'
      );
      var validationRestrictions = JSON.parse(
        validationRestrictionsInput.val()
      );
